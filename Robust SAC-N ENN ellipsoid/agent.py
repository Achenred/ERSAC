...
class SACN(nn.Module):
    ...
    def calc_policy_loss(self, states, done, beta):
        ...
        z = torch.randn(self.num_critics, states.size(0), self.z_dim, device=states.device)

        # Compute Q-values for the actions from the critics
        q_values = self.critic(states, action, z)  # Shape: (num_critics, batch_size, num_actions)

        # Compute mean and covariance
        q_mean = q_values.mean(dim=0)  # Shape: (batch_size, num_actions)
        centered = q_values - q_mean.unsqueeze(0)  # Shape: (num_critics, batch_size, num_actions)
        q_cov = torch.einsum('ijk,ijl->jkl', centered, centered) / q_values.size(
            0)  # Shape: (batch_size, num_actions, num_actions)
    ...
    def calc_policy_loss1(self, states, done, alpha):
        ...
        z = torch.randn(self.num_critics, states.size(0), self.z_dim,
                        device=states.device)  # [num_critics, batch_size, z_dim]

        q_value_dist = self.critic(states, action, z)
        ...
        temp_replicated = temp_unsqueezed.expand(self.num_critics, -1, -1)  # Shape becomes [10, 252, 2]
    ...
    def _critic_loss(
        ...
        with (torch.no_grad()):
            ...
            q_next = self.target_critic(next_state, next_action,z)  #.min(0).values

            # Compute mean and covariance across ensemble dimension
            q_next_mean = q_next.mean(dim=0)  # Shape: (batch_size, num_actions)
            centered = q_next - q_next_mean.unsqueeze(0)  # Shape: (num_critics, batch_size, num_actions)
            q_next_cov = torch.einsum('ijk,ijl->jkl', centered, centered) / q_next.size(
                0)  # Shape: (batch_size, num_actions, num_actions)
        ...
        q_values = self.critic(state, action,z)  # Shape: (num_critics, batch_size, num_actions)

        # Gather Q-values for the selected actions
        action_expanded = action.unsqueeze(0).expand(q_values.size(0), -1, -1)  # Shape: (num_critics, batch_size, 1)
        q_values_gathered = q_values.gather(2, action_expanded.long()) #.squeeze(-1)  # Shape: (num_critics, batch_size)
        ...
        loss = loss.mean(dim=(1)).sum()  # Mean over batch, sum over critics
