...
def train_sac_n(actor, critics, target_critics, data, env, *,
    ...
    M = 300 # plateau patience (eval rounds)
    ...
    if load_pretrained and os.path.isfile(actor_ckpt):
        actor.load_state_dict(torch.load(actor_ckpt, map_location=device))
        print(f"loaded pretrained weights from '{actor_ckpt}'")
    ...
    for epoch in range(num_epochs):
        ...
        pi = actor(s)
        qs = critics(s)
        min_q = torch.min(qs, dim=0).values.detach()
...
if __name__ == "__main__":
    ...
    models = [ "ellipsoid"] #, "ellipsoid" ,"box", "convex_hull"]  # "ellipsoid", "convex_hull","epinet_nl",
    data_sizes =   [1_000,10_000,100_000]  #, 10_000, 100_000]
    tau_values = [0.1]  #[0.1, 0.5, 0.9]
    seeds = [0]
    ...
    for env_name, model_type, data_size, tau, seed in product(
        ...
        if model_type == "box":
            ...
            trained_actor, trained_critic = train_sac_n(
                ...
                actor_lr=1e-4, critic_lr=2e-4,
                batch_size=256, num_epochs=5000, up = 0.005,
            ...
            trained_actor, trained_critic = train_sac_n_CH(
                ...
                batch_size=256, num_epochs=5000, polyak= 0.005,
                eval_interval=1, wandb_log=True)
