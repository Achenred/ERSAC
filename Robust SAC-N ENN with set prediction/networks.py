...
class EpistemicCritic(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, z_dim: int, hidden_dim: int, num_critics: int):
        ...
        self.learnable_epinet = nn.Sequential(
            nn.Linear(hidden_dim , action_dim*z_dim),
            # nn.ReLU(),
            # nn.Linear(hidden_dim, hidden_dim),
            # nn.ReLU(),
    ...
    def forward(self, state: torch.Tensor,action_dim, z: torch.Tensor) -> torch.Tensor:
        ...
        # Compute learnable epinet output: σηL(x ̃)
        learnable_output = self.learnable_epinet(features)  # [batch_size, action_dim * z_dim]
        learnable_output = learnable_output.view(-1, base_output.size(1), self.z_dim)  # [batch_size, action_dim, z_dim]

        # Compute prior epinet output: σP(x ̃)
        prior_output = self.prior_epinet(features)  # [batch_size, action_dim * z_dim]
        prior_output = prior_output.view(-1, base_output.size(1), self.z_dim)  # [batch_size, action_dim, z_dim]

        # Combine learnable and prior outputs
        sigma = learnable_output + prior_output  # [batch_size, action_dim, z_dim]
        # Compute σ^T z for each sample
        epinet_output = torch.bmm(sigma, z.unsqueeze(-1)).squeeze(-1)  # [batch_size, action_dim]


        # ENN output: fθ(x, z) = μζ(x) + ση(x ̃, z)
        base_output_repeated = base_output #.unsqueeze(0).repeat(self.num_critics, 1, 1)  # [num_critics, batch_size, action_dim]
        q_values = base_output_repeated + epinet_output  # [num_critics, batch_size, action_dim]
        q_mean = base_output
        q_cov = torch.matmul(sigma, sigma.transpose(-1, -2))  # Shape: (256, 2, 2)

        return q_values, q_mean, q_cov