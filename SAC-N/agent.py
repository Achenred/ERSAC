...
class SACN(nn.Module):
    ...
    def calc_policy_loss(self, states, alpha):
        ...
        action, action_probs, log_pis = self.actor_local.evaluate(states)
        ...
        # temp = (alpha.to(self.device) * log_pis - q_value_min)
        # loss = torch.mean(torch.matmul(action_probs.T, temp))

        # loss = (1/q_value_min.shape[0])*(self.alpha * log_pis - q_value_min).sum()
        loss = (action_probs * (alpha.to(self.device) * log_pis - q_value_min)).sum(1).mean()
        # loss = (self.alpha * log_pis - q_value_min).mean()
    ...
    def _critic_loss(
        ...
        with torch.no_grad():
            next_action, next_action_probs, next_action_log_prob = self.actor_local.evaluate(next_state)

            q_next = self.target_critic(next_state, next_action).min(0).values
            # q_next = q_next - self.alpha * next_action_log_prob

            # q_next = next_action_probs * (q_next - self.alpha.to(self.device) * next_action_log_prob)
            q_next = next_action_probs*(q_next - self.alpha.to(self.device) * next_action_log_prob)

            # assert q_next.unsqueeze(-1).shape == done.shape == reward.shape

            q_target = reward + self.gamma * (1 - done) * q_next.sum(dim=1).unsqueeze(-1)
        ...
        self.eta = 1.0
        if self.eta > 0:
            obs_tile = state  #.unsqueeze(0).repeat(self.num_critics, 1, 1)
            actions_tile = action.unsqueeze(0).repeat(self.num_critics, 1, 1).requires_grad_(True)
            qs_preds_tile = self.critic(obs_tile, actions_tile)

            qs_preds_selected = torch.gather(qs_preds_tile, dim=2, index= actions_tile.long())

            qs_pred_grads, = torch.autograd.grad(qs_preds_selected.sum(), actions_tile, retain_graph=True,
            ...
            masks = torch.eye(self.num_critics, device= self.device).unsqueeze(dim=0).repeat(qs_pred_grads.size(0), 1, 1)
            qs_pred_grads = (1 - masks) * qs_pred_grads
            grad_loss = torch.mean(torch.sum(qs_pred_grads, dim=(1, 2))) / (self.num_critics - 1)

            loss += self.eta * grad_loss
