...
class ModelBasedRL(RLBase):
    ...
    def dynamic_risk_model(self,mdp, risk_tau=0.75, max_iter=1000):
        """Dynamic risk-based Q-iteration model.

        :param mdp: Markov decision process object containing states, actions, transition probabilities, and rewards.
        :param gamma: Discount factor.
        :param risk_tau: Expectile risk parameter.
        :param max_iter: Maximum number of iterations for Q-iteration.
        :return: Optimal Q-values and derived policy.
        """
        ...
        # Initialize Q-values
        Q = np.zeros((mdp.n, mdp.m))
        Q_ = np.zeros((mdp.n, mdp.m))
        hs = []
        ...
        for it in range(max_iter):
            for s in mdp.states:
                for a in mdp.actions:
                    P_sa = mdp.P[s, a, :]
                    Q_sa = mdp.R[s, a] + self.gamma * np.max(Q, axis=1)
                    Q_[s, a] = rho_f(P_sa, Q_sa)

            hs.append(np.linalg.norm(Q - Q_))
            Q = Q_.copy()

        self.policy = np.zeros((mdp.n, mdp.m),dtype=int)
        self.action_vals = Q

        # Derive policy from Q-values
        for s in range(mdp.n):
            self.policy[s, np.argmax(Q[s])] = 1.0
...
class ModelFreeRL(ModelFreeRLBase):
    def __init__(self,  gamma=1, alpha=.618, alpha_decay=True,
        ...
        super(ModelFreeRL, self).__init__( gamma, alpha,
...
class GridDisplay(object):
    ...
    def show_q_values(self, title='Grid World', fig_path=None, fig_name=None, save_fig=True):
        ...
        count = 0
        for p, value, choice in zip(grid.get_paths(), grid.get_array(), self.policy):
            ...
            for v in range(self.grid_object.m):
                ...
                patch = patches.PathPatch(path, edgecolor='white',
                                          facecolor=cmap((self.q_values[count][a_change[v]] - min_v) / (max_v - min_v)),
                                          lw=4)
                ax.add_patch(patch)
